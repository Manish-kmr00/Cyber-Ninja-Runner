package com.json;

import android.graphics.Bitmap;
import android.os.Handler;
import android.webkit.WebResourceError;
import android.webkit.WebResourceRequest;
import android.webkit.WebResourceResponse;
import android.webkit.WebSettings;
import android.webkit.WebView;
import android.webkit.WebViewClient;
import com.amazon.aps.shared.metrics.model.ApsMetricsDataMap;
import com.fyber.inneractive.sdk.external.InneractiveMediationDefs;
import com.json.mediationsdk.IronSource;
import com.json.mediationsdk.p;
import com.json.mediationsdk.testSuite.TestSuiteActivity;
import com.mbridge.msdk.foundation.download.core.DownloadCommon;
import com.safedk.android.analytics.brandsafety.BrandSafetyUtils;
import com.safedk.android.analytics.brandsafety.creatives.CreativeInfoManager;
import com.safedk.android.internal.partials.IronSourceNetworkBridge;
import com.safedk.android.utils.Logger;
import com.safedk.android.utils.h;
import java.util.Iterator;
import java.util.List;
import kotlin.Metadata;
import kotlin.collections.CollectionsKt;
import kotlin.jvm.internal.Intrinsics;
import kotlin.text.StringsKt;
import org.json.JSONObject;

/* JADX INFO: loaded from: classes9.dex */
@Metadata(d1 = {"\u0000Z\n\u0002\u0018\u0002\n\u0002\u0018\u0002\n\u0002\u0018\u0002\n\u0000\n\u0002\u0018\u0002\n\u0000\n\u0002\u0010\u000e\n\u0000\n\u0002\u0010\u0002\n\u0000\n\u0002\u0018\u0002\n\u0002\b\u0003\n\u0002\u0018\u0002\n\u0000\n\u0002\u0010 \n\u0002\u0010\u0000\n\u0002\b\u0005\n\u0002\u0018\u0002\n\u0002\b\u0005\n\u0002\u0018\u0002\n\u0002\b\u0002\n\u0002\u0010\u000b\n\u0002\b\u0005\n\u0002\u0018\u0002\n\u0002\b\u0007\u0018\u00002\u00020\u0001B)\u0012\u0006\u0010\u0003\u001a\u00020\u0002\u0012\u0006\u0010\u0005\u001a\u00020\u0004\u0012\u0006\u0010*\u001a\u00020\u001d\u0012\b\u0010\u0007\u001a\u0004\u0018\u00010\u0006¢\u0006\u0004\b+\u0010,J\"\u0010\t\u001a\u00020\b2\u0006\u0010\u0003\u001a\u00020\u00022\u0006\u0010\u0005\u001a\u00020\u00042\b\u0010\u0007\u001a\u0004\u0018\u00010\u0006H\u0003J\b\u0010\u000b\u001a\u0004\u0018\u00010\nJ\u0006\u0010\f\u001a\u00020\nJ(\u0010\t\u001a\u00020\b2\u0006\u0010\r\u001a\u00020\u00062\u0006\u0010\u000f\u001a\u00020\u000e2\u000e\u0010\u0012\u001a\n\u0012\u0006\u0012\u0004\u0018\u00010\u00110\u0010H\u0016J\u000e\u0010\t\u001a\u00020\b2\u0006\u0010\u0013\u001a\u00020\u0006J\u001e\u0010\t\u001a\u00020\u00062\u0006\u0010\u0014\u001a\u00020\u00062\u000e\u0010\u0015\u001a\n\u0012\u0006\u0012\u0004\u0018\u00010\u00110\u0010J\u0006\u0010\u0016\u001a\u00020\bJ\u0006\u0010\t\u001a\u00020\bR\u0014\u0010\u0019\u001a\u00020\u00178\u0002X\u0082\u0004¢\u0006\u0006\n\u0004\b\t\u0010\u0018R\u0018\u0010\u001b\u001a\u0004\u0018\u00010\n8\u0002@\u0002X\u0082\u000e¢\u0006\u0006\n\u0004\b\u0016\u0010\u001aR\u0014\u0010\u001c\u001a\u00020\n8\u0002X\u0082\u0004¢\u0006\u0006\n\u0004\b\f\u0010\u001aR\u0014\u0010\u001f\u001a\u00020\u001d8\u0002X\u0082\u0004¢\u0006\u0006\n\u0004\b\u000b\u0010\u001eR\u0016\u0010#\u001a\u00020 8\u0002@\u0002X\u0082\u000e¢\u0006\u0006\n\u0004\b!\u0010\"R\u0016\u0010%\u001a\u00020 8\u0002@\u0002X\u0082\u000e¢\u0006\u0006\n\u0004\b$\u0010\"R\u0014\u0010)\u001a\u00020&8\u0002X\u0082\u0004¢\u0006\u0006\n\u0004\b'\u0010(¨\u0006-"}, d2 = {"Lcom/ironsource/ru;", "Lcom/ironsource/du;", "Lcom/ironsource/mediationsdk/testSuite/TestSuiteActivity;", "activity", "Lcom/ironsource/gi;", "uiLifeCycleListener", "", "controllerUrl", "", "a", "Landroid/webkit/WebView;", "d", ApsMetricsDataMap.APSMETRICS_FIELD_CUSTOM, "callbackName", "Lcom/ironsource/mediationsdk/IronSource$AD_UNIT;", "adUnit", "", "", "args", "script", "methodName", "methodArgs", DownloadCommon.DOWNLOAD_REPORT_FIND_FILE_RESULT_VALUE_B, "Landroid/os/Handler;", "Landroid/os/Handler;", "mHandler", "Landroid/webkit/WebView;", "mLoadingScreenWebView", "mAppWebView", "Lorg/json/JSONObject;", "Lorg/json/JSONObject;", "mJsonData", "", "e", "Z", "mInjectData", InneractiveMediationDefs.GENDER_FEMALE, "mLoadingScreenMinimumTimeExceeded", "Lcom/ironsource/wh;", "g", "Lcom/ironsource/wh;", "sdkConfigService", "jsonData", "<init>", "(Lcom/ironsource/mediationsdk/testSuite/TestSuiteActivity;Lcom/ironsource/gi;Lorg/json/JSONObject;Ljava/lang/String;)V", "mediationsdk_release"}, k = 1, mv = {1, 8, 0})
public final class ru implements du {

    /* JADX INFO: renamed from: a, reason: collision with root package name and from kotlin metadata */
    private final Handler mHandler;

    /* JADX INFO: renamed from: b, reason: from kotlin metadata */
    private WebView mLoadingScreenWebView;

    /* JADX INFO: renamed from: c, reason: from kotlin metadata */
    private final WebView mAppWebView;

    /* JADX INFO: renamed from: d, reason: from kotlin metadata */
    private final JSONObject mJsonData;

    /* JADX INFO: renamed from: e, reason: from kotlin metadata */
    private boolean mInjectData;

    /* JADX INFO: renamed from: f, reason: from kotlin metadata */
    private boolean mLoadingScreenMinimumTimeExceeded;

    /* JADX INFO: renamed from: g, reason: from kotlin metadata */
    private final wh sdkConfigService;

    @Metadata(d1 = {"\u0000)\n\u0000\n\u0002\u0018\u0002\n\u0002\u0018\u0002\n\u0000\n\u0002\u0010\u000e\n\u0000\n\u0002\u0010\u0002\n\u0000\n\u0002\u0018\u0002\n\u0000\n\u0002\u0018\u0002\n\u0002\b\u0003*\u0001\u0000\b\n\u0018\u00002\u00020\u0001J\u001c\u0010\u0007\u001a\u00020\u00062\b\u0010\u0003\u001a\u0004\u0018\u00010\u00022\b\u0010\u0005\u001a\u0004\u0018\u00010\u0004H\u0016J \u0010\f\u001a\u00020\u00062\u0006\u0010\u0003\u001a\u00020\u00022\u0006\u0010\t\u001a\u00020\b2\u0006\u0010\u000b\u001a\u00020\nH\u0016¨\u0006\r"}, d2 = {"com/ironsource/ru$a", "Landroid/webkit/WebViewClient;", "Landroid/webkit/WebView;", "view", "", "url", "", "onPageFinished", "Landroid/webkit/WebResourceRequest;", "request", "Landroid/webkit/WebResourceError;", "error", "onReceivedError", "mediationsdk_release"}, k = 1, mv = {1, 8, 0})
    public static final class a extends WebViewClient {
        final /* synthetic */ String b;

        a(String str) {
            this.b = str;
        }

        @Override // android.webkit.WebViewClient
        public void onLoadResource(WebView view, String url) {
            super.onLoadResource(view, url);
            CreativeInfoManager.onResourceLoaded(h.f, view, url);
        }

        @Override // android.webkit.WebViewClient
        public void onPageFinished(WebView view, String url) {
            Logger.d("IronSource|SafeDK: Execution> Lcom/ironsource/ru$a;->onPageFinished(Landroid/webkit/WebView;Ljava/lang/String;)V");
            BrandSafetyUtils.onWebViewPageFinished(h.f, view, url);
            safedk_ru$a_onPageFinished_fc3b4e5db624b7a2c549c54e8004b918(view, url);
        }

        @Override // android.webkit.WebViewClient
        public void onPageStarted(WebView view, String url, Bitmap favicon) {
            super.onPageStarted(view, url, favicon);
            BrandSafetyUtils.onWebViewPageStarted(h.f, view, url);
        }

        @Override // android.webkit.WebViewClient
        public void onReceivedError(WebView view, WebResourceRequest request, WebResourceError error) {
            Logger.d("IronSource|SafeDK: Execution> Lcom/ironsource/ru$a;->onReceivedError(Landroid/webkit/WebView;Landroid/webkit/WebResourceRequest;Landroid/webkit/WebResourceError;)V");
            BrandSafetyUtils.onWebViewReceivedError(h.f, view, request, error);
            safedk_ru$a_onReceivedError_1b4b4616eb0f0089c86446f350ac57a3(view, request, error);
        }

        public void safedk_ru$a_onPageFinished_fc3b4e5db624b7a2c549c54e8004b918(WebView p0, String p1) {
            if (!ru.this.mInjectData && p1 != null && StringsKt.startsWith$default(p1, String.valueOf(this.b), false, 2, (Object) null)) {
                ru ruVar = ru.this;
                List<? extends Object> listListOf = CollectionsKt.listOf(ruVar.mJsonData);
                Intrinsics.checkNotNull(listListOf, "null cannot be cast to non-null type kotlin.collections.List<kotlin.Any?>");
                ruVar.a(ruVar.a("onDataReady", listListOf));
                zt.f4665a.d();
                ru.this.mInjectData = true;
            }
            super.onPageFinished(p0, p1);
        }

        public void safedk_ru$a_onReceivedError_1b4b4616eb0f0089c86446f350ac57a3(WebView view, WebResourceRequest request, WebResourceError error) {
            Intrinsics.checkNotNullParameter(view, "view");
            Intrinsics.checkNotNullParameter(request, "request");
            Intrinsics.checkNotNullParameter(error, "error");
            zt.f4665a.a(error.toString());
            lu.f3878a.b("got error during TestSuite web controller loading " + error);
            super.onReceivedError(view, request, error);
        }

        @Override // android.webkit.WebViewClient
        public WebResourceResponse shouldInterceptRequest(WebView view, WebResourceRequest webResourceRequest) {
            return CreativeInfoManager.onWebViewResponseWithHeaders(h.f, view, webResourceRequest, super.shouldInterceptRequest(view, webResourceRequest));
        }

        @Override // android.webkit.WebViewClient
        public boolean shouldOverrideUrlLoading(WebView view, String url) {
            boolean zShouldOverrideUrlLoading = super.shouldOverrideUrlLoading(view, url);
            BrandSafetyUtils.onShouldOverrideUrlLoading(h.f, view, url, zShouldOverrideUrlLoading);
            return zShouldOverrideUrlLoading;
        }
    }

    @Metadata(d1 = {"\u0000\u0011\n\u0000\n\u0002\u0018\u0002\n\u0002\u0010\u0002\n\u0002\b\u0003*\u0001\u0000\b\n\u0018\u00002\u00020\u0001J\b\u0010\u0003\u001a\u00020\u0002H\u0016J\b\u0010\u0004\u001a\u00020\u0002H\u0016¨\u0006\u0005"}, d2 = {"com/ironsource/ru$b", "Lcom/ironsource/gi;", "", "onUIReady", "onClosed", "mediationsdk_release"}, k = 1, mv = {1, 8, 0})
    public static final class b implements gi {
        final /* synthetic */ gi b;

        b(gi giVar) {
            this.b = giVar;
        }

        /* JADX INFO: Access modifiers changed from: private */
        public static final void a(b this$0) {
            Intrinsics.checkNotNullParameter(this$0, "this$0");
            this$0.onUIReady();
        }

        @Override // com.json.gi
        public void onClosed() {
            this.b.onClosed();
        }

        @Override // com.json.gi
        public void onUIReady() {
            if (ru.this.mLoadingScreenMinimumTimeExceeded) {
                this.b.onUIReady();
            } else {
                ru.this.mHandler.postDelayed(new Runnable() { // from class: com.ironsource.ru$b$$ExternalSyntheticLambda0
                    @Override // java.lang.Runnable
                    public final void run() {
                        ru.b.a(this.f$0);
                    }
                }, 500L);
            }
        }
    }

    public ru(TestSuiteActivity activity, gi uiLifeCycleListener, JSONObject jsonData, String str) {
        Intrinsics.checkNotNullParameter(activity, "activity");
        Intrinsics.checkNotNullParameter(uiLifeCycleListener, "uiLifeCycleListener");
        Intrinsics.checkNotNullParameter(jsonData, "jsonData");
        this.mHandler = new Handler(activity.getMainLooper());
        this.mLoadingScreenWebView = new WebView(activity);
        this.mAppWebView = new WebView(activity);
        this.mJsonData = jsonData;
        this.sdkConfigService = mm.INSTANCE.d().t();
        a(activity, uiLifeCycleListener, str);
    }

    /* JADX INFO: Access modifiers changed from: private */
    public static final void a(gi uiLifeCycleListener) {
        Intrinsics.checkNotNullParameter(uiLifeCycleListener, "$uiLifeCycleListener");
        uiLifeCycleListener.onUIReady();
    }

    private final void a(TestSuiteActivity activity, final gi uiLifeCycleListener, String controllerUrl) {
        WebView webView = this.mLoadingScreenWebView;
        WebSettings settings = webView != null ? webView.getSettings() : null;
        if (settings != null) {
            settings.setJavaScriptEnabled(true);
        }
        this.mAppWebView.getSettings().setJavaScriptEnabled(true);
        List<? extends IronSource.AD_UNIT> listMutableListOf = CollectionsKt.mutableListOf(IronSource.AD_UNIT.BANNER, IronSource.AD_UNIT.INTERSTITIAL, IronSource.AD_UNIT.REWARDED_VIDEO);
        if (this.sdkConfigService.a() != null) {
            Iterator<IronSource.AD_UNIT> it = p.m().p().iterator();
            while (it.hasNext()) {
                listMutableListOf.remove(it.next());
            }
        } else {
            listMutableListOf.clear();
        }
        IronSourceNetworkBridge.onAddedJavascriptInterface(this.mAppWebView, new cu(new yt.a().a(listMutableListOf, activity, this.mHandler), new b(uiLifeCycleListener), this), "TestSuiteBridge");
        this.mAppWebView.setWebViewClient(new a(controllerUrl));
        if (controllerUrl != null && controllerUrl.length() != 0) {
            IronSourceNetworkBridge.webviewLoadUrl(this.mAppWebView, controllerUrl);
        }
        WebView webView2 = this.mLoadingScreenWebView;
        if (webView2 != null) {
            IronSourceNetworkBridge.webviewLoadUrl(webView2, "javascript:document.write(atob(''))");
        }
        this.mHandler.postDelayed(new Runnable() { // from class: com.ironsource.ru$$ExternalSyntheticLambda1
            @Override // java.lang.Runnable
            public final void run() {
                ru.e(this.f$0);
            }
        }, 2000L);
        this.mHandler.postDelayed(new Runnable() { // from class: com.ironsource.ru$$ExternalSyntheticLambda2
            @Override // java.lang.Runnable
            public final void run() {
                ru.a(uiLifeCycleListener);
            }
        }, 5000L);
    }

    /* JADX INFO: Access modifiers changed from: private */
    public static final void a(ru this$0, String script) {
        Intrinsics.checkNotNullParameter(this$0, "this$0");
        Intrinsics.checkNotNullParameter(script, "$script");
        IronSourceNetworkBridge.webviewLoadUrl(this$0.mAppWebView, "javascript:" + script);
    }

    /* JADX INFO: Access modifiers changed from: private */
    public static final void e(ru this$0) {
        Intrinsics.checkNotNullParameter(this$0, "this$0");
        this$0.mLoadingScreenMinimumTimeExceeded = true;
    }

    public final String a(String methodName, List<? extends Object> methodArgs) {
        Intrinsics.checkNotNullParameter(methodName, "methodName");
        Intrinsics.checkNotNullParameter(methodArgs, "methodArgs");
        String str = "TestSuiteJs." + methodName;
        String strB = qu.f4209a.b(methodArgs);
        return (strB.length() > 0 ? new StringBuilder().append(str).append('(').append(strB).append(')') : new StringBuilder().append(str).append("()")).toString();
    }

    public final void a() {
        b();
        this.mAppWebView.destroy();
    }

    public final void a(final String script) {
        Intrinsics.checkNotNullParameter(script, "script");
        this.mHandler.post(new Runnable() { // from class: com.ironsource.ru$$ExternalSyntheticLambda0
            @Override // java.lang.Runnable
            public final void run() {
                ru.a(this.f$0, script);
            }
        });
    }

    @Override // com.json.du
    public void a(String callbackName, IronSource.AD_UNIT adUnit, List<? extends Object> args) {
        Intrinsics.checkNotNullParameter(callbackName, "callbackName");
        Intrinsics.checkNotNullParameter(adUnit, "adUnit");
        Intrinsics.checkNotNullParameter(args, "args");
        if (this.mInjectData) {
            String string = adUnit.toString();
            Intrinsics.checkNotNullExpressionValue(string, "adUnit.toString()");
            a(a("onCallbackReceived", CollectionsKt.listOf(callbackName, string, args)));
        }
    }

    public final void b() {
        WebView webView = this.mLoadingScreenWebView;
        if (webView != null) {
            if (webView != null) {
                webView.destroy();
            }
            this.mLoadingScreenWebView = null;
        }
    }

    /* JADX INFO: renamed from: c, reason: from getter */
    public final WebView getMAppWebView() {
        return this.mAppWebView;
    }

    /* JADX INFO: renamed from: d, reason: from getter */
    public final WebView getMLoadingScreenWebView() {
        return this.mLoadingScreenWebView;
    }
}
