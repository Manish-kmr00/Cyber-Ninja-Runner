...
public final class SliderIndicatorAnimator implements IndicatorAnimator {
    ...
    public RectF getSelectedItemRect(float xOffset, float yOffset, float viewportWidth, boolean isLayoutRtl) {
        ...
        if (isLayoutRtl) {
            ...
            float f2 = width / 2.0f;
            rectF.left = (xOffset - RangesKt.coerceAtMost(this.selectedPositionOffset * f, f)) - f2;
            this.itemRect.right = (xOffset - RangesKt.coerceAtLeast(this.spaceBetweenCenters * this.selectedPositionOffset, 0.0f)) + f2;
        } else {
            float f3 = width / 2.0f;
            this.itemRect.left = (RangesKt.coerceAtLeast(this.spaceBetweenCenters * this.selectedPositionOffset, 0.0f) + xOffset) - f3;
            RectF rectF2 = this.itemRect;
            float f4 = this.spaceBetweenCenters;
            rectF2.right = xOffset + RangesKt.coerceAtMost(this.selectedPositionOffset * f4, f4) + f3;
        }
