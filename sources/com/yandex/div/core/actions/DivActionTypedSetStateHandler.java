...
public final class DivActionTypedSetStateHandler implements DivActionTypedHandler {
    ...
    private final void handleSetState(DivActionSetState action, Div2View view, ExpressionResolver resolver) {
        String strEvaluate = action.stateId.evaluate(resolver);
        try {
            view.switchToState(DivStatePath.INSTANCE.parse(strEvaluate), action.temporary.evaluate(resolver).booleanValue());
        } catch (PathFormatException e) {
            DivActionTypedUtilsKt.logError(view, new IllegalArgumentException("Invalid format of " + strEvaluate, e));
        }
    }
}
