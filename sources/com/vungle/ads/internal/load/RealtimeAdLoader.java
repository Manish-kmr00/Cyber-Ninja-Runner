...
public final class RealtimeAdLoader extends BaseAdLoader {
        ...
        if (ConfigManager.INSTANCE.rtaDebuggingEnabled()) {
            try {
                ...
                if (decodedAdsResponse != null) {
                    new RTADebugger(m6053requestAd$lambda0(lazy)).reportAdMarkup(decodedAdsResponse);
                }
    ...
    private static final VungleApiClient m6053requestAd$lambda0(Lazy<VungleApiClient> lazy) {
    ...
    private final void sendWinNotification(List<String> notifications) {
        if (notifications == null || !notifications.isEmpty()) {
            ServiceLocator.Companion companion = ServiceLocator.INSTANCE;
            final Context context = getContext();
            TpatSender tpatSender = new TpatSender(getVungleApiClient(), getLogEntry(), getSdkExecutors().getIO_EXECUTOR(), getPathProvider(), m6054sendWinNotification$lambda2(LazyKt.lazy(LazyThreadSafetyMode.SYNCHRONIZED, (Function0) new Function0<SignalManager>() { // from class: com.vungle.ads.internal.load.RealtimeAdLoader$sendWinNotification$$inlined$inject$1
                /* JADX WARN: 'super' call moved to the top of the method (can break code semantics) */
                {
                    super(0);
                }
    ...
    private static final SignalManager m6054sendWinNotification$lambda2(Lazy<SignalManager> lazy) {
        return lazy.getValue();
    }
}
