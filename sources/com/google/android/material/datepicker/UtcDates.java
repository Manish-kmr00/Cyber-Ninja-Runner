...
class UtcDates {
    ...
    static String getTextInputHint(Resources resources, SimpleDateFormat simpleDateFormat) {
        ...
        if (pattern.replaceAll("[^y]", "").length() == 1) {
            pattern = pattern.replace(VastAttributes.VERTICAL_POSITION, "yyyy");
        }
        return pattern.replace("d", string3).replace("M", string2).replace(VastAttributes.VERTICAL_POSITION, string);
    }
