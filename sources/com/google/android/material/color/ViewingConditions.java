...
final class ViewingConditions {
    ...
    static ViewingConditions make(float[] fArr, float f, float f2, float f3, boolean z) {
        ...
        if (f12 >= 0.9d) {
            fLerp = MathUtils.lerp(0.59f, 0.69f, (f12 - 0.9f) * 10.0f);
        } else {
            fLerp = MathUtils.lerp(0.525f, 0.59f, (f12 - 0.8f) * 10.0f);
        }
        float f13 = fLerp;
        ...
        float f16 = 1.0f - f15;
        float fCbrt = (f15 * f) + (0.1f * f16 * f16 * ((float) Math.cbrt(((double) f) * 5.0d)));
        ...
        return new ViewingConditions(fYFromLstar, ((fArr8[0] * 2.0f) + fArr8[1] + (fArr8[2] * 0.05f)) * fPow, fPow, fPow, f13, f12, fArr6, fCbrt, (float) Math.pow(fCbrt, 0.25d), fSqrt);
    }
