...
final class Cam16 {
    ...
    static Cam16 fromIntInViewingConditions(int i, ViewingConditions viewingConditions) {
        ...
        float fPow = (float) Math.pow(((double) (viewingConditions.getFl() * Math.abs(f7))) / 100.0d, 0.42d);
        float fPow2 = (float) Math.pow(((double) (viewingConditions.getFl() * Math.abs(f8))) / 100.0d, 0.42d);
        float fPow3 = (float) Math.pow(((double) (viewingConditions.getFl() * Math.abs(f9))) / 100.0d, 0.42d);
        ...
        double d2 = f11;
        double d3 = f10;
        float fAtan2 = (((float) Math.atan2(d2, d3)) * 180.0f) / 3.1415927f;
        if (fAtan2 < 0.0f) {
            fAtan2 += 360.0f;
        ...
        float fPow4 = ((float) Math.pow((f14 * viewingConditions.getNbb()) / viewingConditions.getAw(), viewingConditions.getC() * viewingConditions.getZ())) * 100.0f;
        ...
        float fPow5 = ((float) Math.pow(1.64d - Math.pow(0.29d, viewingConditions.getN()), 0.73d)) * ((float) Math.pow((((((((float) (Math.cos(Math.toRadians(((double) fAtan2) < 20.14d ? 360.0f + fAtan2 : fAtan2) + 2.0d) + 3.8d)) * 0.25f) * 3846.1538f) * viewingConditions.getNc()) * viewingConditions.getNcb()) * ((float) Math.hypot(d3, d2))) / (f13 + 0.305f), 0.9d));
        ...
        double d4 = f15;
        return new Cam16(fAtan2, fSqrt, fPow4, c, flRoot, fSqrt2, f16, fLog1p * ((float) Math.cos(d4)), fLog1p * ((float) Math.sin(d4)));
    }
    ...
    private static Cam16 fromJchInViewingConditions(float f, float f2, float f3, ViewingConditions viewingConditions) {
        ...
        return new Cam16(f3, f2, f, c, flRoot, fSqrt, f4, fLog1p * ((float) Math.cos(d2)), fLog1p * ((float) Math.sin(d2)));
    }
