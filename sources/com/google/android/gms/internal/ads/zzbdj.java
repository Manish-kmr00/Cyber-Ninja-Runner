...
public final class zzbdj {
            ...
            if (listQueryIntentActivities != null && resolveInfoResolveActivity != null) {
                for (int i = 0; i < listQueryIntentActivities.size(); i++) {
                    if (resolveInfoResolveActivity.activityInfo.name.equals(listQueryIntentActivities.get(i).activityInfo.name)) {
                        return resolveInfoResolveActivity.activityInfo.packageName.equals(zzhfo.zza(context));
                    }
                }
            }
        }
    ...
    public final void zzb(Activity activity) {
        String strZza;
        if (this.zzb == null && (strZza = zzhfo.zza(activity)) != null) {
            zzhfp zzhfpVar = new zzhfp(this);
            this.zzc = zzhfpVar;
            CustomTabsClient.bindCustomTabsService(activity, strZza, zzhfpVar);
        }
    }
