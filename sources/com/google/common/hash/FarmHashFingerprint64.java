...
final class FarmHashFingerprint64 extends AbstractNonStreamingHashFunction {
    ...
    private static long hashLength17to32(byte[] bytes, int offset, int length) {
        ...
        return hashLength16((LittleEndianByteArray.load64(bytes, i - 16) * K2) + Long.rotateRight(jLoad64 + jLoad65, 43) + Long.rotateRight(jLoad66, 30), jLoad66 + jLoad64 + Long.rotateRight(jLoad65 + K2, 18), j);
    }
    ...
    private static long hashLength33To64(byte[] bytes, int offset, int length) {
        ...
        return hashLength16(((jHashLength16 + LittleEndianByteArray.load64(bytes, i - 24)) * j) + Long.rotateRight(jLoad67 + jLoad68, 43) + Long.rotateRight(jLoad69, 30), jLoad67 + Long.rotateRight(jLoad68 + jLoad64, 18) + jLoad69, j);
    }
    ...
    private static long hashLength65Plus(byte[] bytes, int offset, int length) {
        ...
        while (true) {
            ...
            weakHashLength32WithSeeds(bytes, i6 + 32, jRotateRight3 + jArr2[1], jLoad65 + LittleEndianByteArray.load64(bytes, i6 + 16), jArr2);
            int i7 = i6 + 64;
            if (i7 == i2) {
                long j4 = K1 + ((j3 & 255) << 1);
                ...
                jArr[0] = j6;
                jArr2[0] = jArr2[0] + j6;
                long jRotateRight4 = Long.rotateRight(jRotateRight3 + jLoad65 + jArr[0] + LittleEndianByteArray.load64(bytes, i4 - 55), 37) * j4;
                long jRotateRight5 = Long.rotateRight(jLoad65 + jArr[1] + LittleEndianByteArray.load64(bytes, i4 - 15), 42) * j4;
                long j7 = jRotateRight4 ^ (jArr2[1] * 9);
                long jLoad66 = jRotateRight5 + (jArr[0] * 9) + LittleEndianByteArray.load64(bytes, i4 - 23);
                long jRotateRight6 = Long.rotateRight(j3 + jArr2[0], 33) * j4;
                weakHashLength32WithSeeds(bytes, i5, jArr[1] * j4, j7 + jArr2[0], jArr);
                weakHashLength32WithSeeds(bytes, i4 - 31, jRotateRight6 + jArr2[1], jLoad66 + LittleEndianByteArray.load64(bytes, i4 - 47), jArr2);
