...
public final class BigIntegerMath {
    ...
    public static int log2(BigInteger x, RoundingMode mode) {
        MathPreconditions.checkPositive(VastAttributes.HORIZONTAL_POSITION, (BigInteger) Preconditions.checkNotNull(x));
        ...
        switch (AnonymousClass1.$SwitchMap$java$math$RoundingMode[mode.ordinal()]) {
            ...
            case 8:
                if (i < 256) {
                    return x.compareTo(SQRT2_PRECOMPUTED_BITS.shiftRight(256 - i)) <= 0 ? i : iBitLength;
                }
                return x.pow(2).bitLength() + (-1) < (i * 2) + 1 ? i : iBitLength;
            default:
    ...
    public static int log10(BigInteger x, RoundingMode mode) {
        int iCompareTo;
        MathPreconditions.checkPositive(VastAttributes.HORIZONTAL_POSITION, x);
        if (fitsInLong(x)) {
    ...
    public static BigInteger sqrt(BigInteger x, RoundingMode mode) {
        MathPreconditions.checkNonNegative(VastAttributes.HORIZONTAL_POSITION, x);
        if (fitsInLong(x)) {
