...
public class WebRequest {
    ...
    public long makeStreamRequest(OutputStream outputStream) throws Exception {
        ...
        if (getRequestType().equals(RequestType.POST.name())) {
            ...
            OutputStream outputStreamUrlConnectionGetOutputStream = null;
            try {
                try {
                    outputStreamUrlConnectionGetOutputStream = UnityCoreNetworkBridge.urlConnectionGetOutputStream(httpUrlConnectionWithHeaders);
                    if (getBody() == null) {
                        String query = getQuery();
                        if (query != null) {
                            outputStreamUrlConnectionGetOutputStream.write(query.getBytes(StandardCharsets.UTF_8));
                        }
                    } else {
                        outputStreamUrlConnectionGetOutputStream.write(getBody());
                    }
