...
public final class C3839w {
    ...
    private int A = 3;
    private CameraCaptureSession.CaptureCallback w = new C3829r(this);
    private final CameraDevice.StateCallback x = new C3833t(this);
    private final ImageReader.OnImageAvailableListener y = new C3835u(this);
    private final SurfaceTexture.OnFrameAvailableListener z = new C3837v(this);

    protected C3839w(com.unity3d.player.a.b bVar) {
        this.f8312a = null;
        this.f8312a = bVar;
        e();
    }
    ...
    public static int a(Context context, int i) {
        try {
            ...
            B.Log(6, "Camera2: CameraAccessException " + e);
        }
        return 0;
    }
    ...
    private void b() {
        try {
            ...
            if (!semaphore.tryAcquire(4L, timeUnit)) {
                B.Log(5, "Camera2: Timeout waiting to lock camera for closing.");
                return;
            }
            this.b.close();
            try {
                if (!D.tryAcquire(4L, timeUnit)) {
                    B.Log(5, "Camera2: Timeout waiting to close camera.");
                }
            } catch (InterruptedException e) {
                B.Log(6, "Camera2: Interrupted while waiting to close camera " + e);
            }
            this.b = null;
            D.release();
        } catch (InterruptedException e2) {
            B.Log(6, "Camera2: Interrupted while trying to lock camera for closing " + e2);
        }
    }
    ...
    public static int[] b(Context context, int i) {
        ...
        try {
            ...
            B.Log(6, "Camera2: configuration map is not available.");
            outputSizes = null;
            if (outputSizes != null) {
                return null;
            }
    ...
    public final void a() {
        ...
        try {
            ...
            this.c.interrupt();
            B.Log(6, "Camera2: Interrupted while waiting for the background thread to finish " + e);
        }
    }
    ...
    public final boolean a(float f, float f2) {
        ...
        if (this.m) {
            B.Log(5, "Camera2: Setting manual focus point already started.");
            return false;
        }
        this.i = f;
        this.j = f2;
    ...
    public final boolean a(Context context, int i, int i2, int i3, int i4, int i5, Surface surface) {
        ...
        try {
            ...
            if (((Integer) cameraCharacteristics.get(CameraCharacteristics.INFO_SUPPORTED_HARDWARE_LEVEL)).intValue() == 2) {
                B.Log(5, "Camera2: only LEGACY hardware level is supported.");
            }
            ...
            if (rangeArr == null || rangeArr.length == 0) {
                B.Log(6, "Camera2: target FPS ranges are not avialable.");
                return false;
            }
            int i10 = -1;
            int i11 = 0;
            ...
            try {
                if (!D.tryAcquire(4L, TimeUnit.SECONDS)) {
                    B.Log(5, "Camera2: Timeout waiting to lock camera for opening.");
                    return false;
                }
                try {
                ...
                try {
                    if (!D.tryAcquire(4L, TimeUnit.SECONDS)) {
                        B.Log(5, "Camera2: Timeout waiting to open camera.");
                        return false;
                    }
                    D.release();
                    this.t = i5;
                    ...
                    B.Log(6, "Camera2: Interrupted while waiting to open camera " + e4);
                }
            } catch (InterruptedException e5) {
                B.Log(6, "Camera2: Interrupted while trying to lock camera for opening " + e5);
                return false;
            }
        } catch (CameraAccessException e6) {
            B.Log(6, "Camera2: CameraAccessException " + e6);
            return false;
        }
    }
    ...
    public void h() {
        ...
        try {
            CameraCaptureSession cameraCaptureSession = this.r;
            if (cameraCaptureSession == null) {
                this.b.createCaptureSession(Arrays.asList(this.v), new C3831s(this), this.d);
            } else if (this.A == 2) {
                cameraCaptureSession.setRepeatingRequest(this.q.build(), this.w, this.d);
            }
            this.A = 1;
