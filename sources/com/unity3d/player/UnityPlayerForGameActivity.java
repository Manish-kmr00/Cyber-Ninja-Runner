...
public class UnityPlayerForGameActivity extends UnityPlayer {
    Thread m_MainThread;
    private M m_PersistentUnitySurface;
    ...
    public UnityPlayerForGameActivity(Activity activity, FrameLayout frameLayout, SurfaceView surfaceView, IUnityPlayerLifecycleEvents iUnityPlayerLifecycleEvents) {
        super(activity, EnumC3841x.c, iUnityPlayerLifecycleEvents);
        this.m_SurfaceView = surfaceView;
        initialize(frameLayout);
        this.m_MainThread = null;
        nativeUnityPlayerForGameActivityInitialized(!A1.d() ? 1 : 0);
        applySurfaceViewSettings(this.m_SurfaceView);
        this.m_PersistentUnitySurface = new M(activity);
        this.m_SurfaceView.getHolder().addCallback(new v1(this));
    }
    ...
    void onOrientationChanged(int i, int i2) {
        invokeOnMainThread((Runnable) new w1(this, i, i2));
    }

    @Override // com.unity3d.player.UnityPlayer
    void pauseUnity() {
        super.pauseUnity();
        invokeOnMainThread((Runnable) new x1(this));
    }

    @Override // com.unity3d.player.UnityPlayer
    void resumeUnity() {
        super.resumeUnity();
        invokeOnMainThread((Runnable) new y1(this));
    }
    ...
    public void setMainSurfaceViewAspectRatio(float f) {
        B.Log(6, "setMainSurfaceViewAspectRatio is not supported for GameActivity");
    }
