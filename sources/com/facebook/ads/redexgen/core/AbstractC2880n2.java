...
public abstract class AbstractC2880n2 {
    ...
    public static int A00(int expectedSize) {
        if (expectedSize < 3) {
            AbstractC2748ko.A00(expectedSize, A07(2, 12, Sdk.SDKError.Reason.INVALID_LOG_ERROR_ENDPOINT_VALUE));
    ...
    public static <V> InterfaceC2570hs<Map.Entry<?, V>, V> A04() {
        O3 o3 = O3.A03;
        if (A01[6].charAt(13) == 's') {
            throw new RuntimeException();
        }
    ...
    public static String A08(Map<?, ?> map) {
        StringBuilder sbAppend = AbstractC2753kt.A01(map.size()).append(AbstractJsonLexerKt.BEGIN_OBJ);
        boolean z = true;
        for (Map.Entry<?, ?> entry : map.entrySet()) {
            ...
            z = false;
            sbAppend.append(entry.getKey()).append(cc.T).append(entry.getValue());
        }
    ...
    public static boolean A0A(@CheckForNull Map<?, ?> map, Object value) {
        return AbstractC2841mO.A0E(A03(map.entrySet().iterator()), value);
    }
