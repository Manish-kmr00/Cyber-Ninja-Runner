...
public final class ReaderJsonLexer extends AbstractJsonLexer {
    ...
    private final void preload(int unprocessedCount) {
        ...
        while (unprocessedCount != length) {
            int i = this.reader.read(cArr, unprocessedCount, length - unprocessedCount);
            if (i == -1) {
    ...
    protected void appendRange(int fromIndex, int toIndex) {
        Intrinsics.checkNotNullExpressionValue(getEscapedString().append(getSource().buffer, fromIndex, toIndex - fromIndex), "this.append(value, start…x, endIndex - startIndex)");
    }
