...
public final class CharsetJVMKt {
    ...
    public static final int encodeImpl(CharsetEncoder charsetEncoder, CharSequence input, int i, int i2, Buffer dst) throws CharacterCodingException {
        ...
        CoderResult result = charsetEncoder.encode(charBufferWrap, byteBufferM7639slice87lwejk, false);
        ...
        dst.commitWritten(byteBufferM7639slice87lwejk.position());
        ...
        try {
                            ...
                            if (charBufferAsCharBuffer.hasRemaining()) {
                                ...
                                while (true) {
                                    try {
                                        ...
                                        ByteBuffer byteBufferM7639slice87lwejk = Memory.m7639slice87lwejk(memory2, writePosition3, limit2);
                                        CoderResult cr = charsetEncoder.encode(charBufferAsCharBuffer, byteBufferM7639slice87lwejk, false);
                                        if (cr.isUnmappable() || cr.isMalformed()) {
                                            Intrinsics.checkNotNullExpressionValue(cr, "cr");
                                            throwExceptionWrapped(cr);
                                        }
                                        i14 = (cr.isOverflow() && byteBufferM7639slice87lwejk.hasRemaining()) ? i14 + 1 : 1;
                                        if (byteBufferM7639slice87lwejk.limit() != limit2) {
                                            throw new IllegalStateException("Buffer's limit change is not allowed".toString());
                                        }
                                        chunkBuffer4.commitWritten(byteBufferM7639slice87lwejk.position());
            ...
            while (true) {
                try {
                    ...
                    ByteBuffer byteBufferM7639slice87lwejk2 = Memory.m7639slice87lwejk(memory3, writePosition4, limit3);
                    CoderResult cr2 = charsetEncoder.encode(charBufferAsCharBuffer, byteBufferM7639slice87lwejk2, true);
                    ...
                    if (byteBufferM7639slice87lwejk2.limit() != limit3) {
                        try {
                            throw new IllegalStateException("Buffer's limit change is not allowed".toString());
                        } catch (Throwable th4) {
                            th = th4;
                    ...
                    chunkBuffer5.commitWritten(byteBufferM7639slice87lwejk2.position());
                    if (i16 <= 0) {
                        dst.afterHeadWrite();
                    ...
                    if (writePosition >= i2) {
                        try {
                            ...
                            if (i4 == 0) {
                                ...
                                ByteBuffer byteBufferM7639slice87lwejk = Memory.m7639slice87lwejk(memory, readPosition, writePosition2);
                                charBufferAllocate.clear();
                                if (i4 < 8192) {
                                    charBufferAllocate.limit(i4);
                                }
                                CoderResult rc = charsetDecoder.decode(byteBufferM7639slice87lwejk, charBufferAllocate, false);
                                ...
                                i3 = (rc.isUnderflow() && byteBufferM7639slice87lwejk.hasRemaining()) ? i3 + 1 : 1;
                                if (byteBufferM7639slice87lwejk.limit() != writePosition2) {
                                    throw new IllegalStateException("Buffer's limit change is not allowed".toString());
                                }
                                chunkBuffer2.discardExact(byteBufferM7639slice87lwejk.position());
                                i2 = i3;
                            }
    ...
    public static final String decodeExactBytes(CharsetDecoder charsetDecoder, Input input, int i) {
        ...
        if (input.getHeadEndExclusive() - input.getHeadPosition() >= i) {
            if (input.getHeadMemory().hasArray()) {
                ByteBuffer byteBufferM7812getHeadMemorySK3TCg8 = input.getHeadMemory();
                byte[] bArrArray = byteBufferM7812getHeadMemorySK3TCg8.array();
                Intrinsics.checkNotNullExpressionValue(bArrArray, "bb.array()");
                int iArrayOffset = byteBufferM7812getHeadMemorySK3TCg8.arrayOffset() + byteBufferM7812getHeadMemorySK3TCg8.position() + input.getHead().getReadPosition();
                Charset charset = charsetDecoder.charset();
                Intrinsics.checkNotNullExpressionValue(charset, "charset()");
    ...
    private static final String decodeImplByteBuffer(CharsetDecoder charsetDecoder, Input input, int i) throws CharacterCodingException {
        ...
        ByteBuffer byteBufferM7639slice87lwejk = Memory.m7639slice87lwejk(input.getHeadMemory(), input.getHead().getReadPosition(), i);
        CoderResult rc = charsetDecoder.decode(byteBufferM7639slice87lwejk, charBufferAllocate, true);
        if (rc.isMalformed() || rc.isUnmappable()) {
            Intrinsics.checkNotNullExpressionValue(rc, "rc");
            throwExceptionWrapped(rc);
        }
        charBufferAllocate.flip();
        input.discardExact(byteBufferM7639slice87lwejk.position());
                    ...
                    if (writePosition >= i2) {
                        try {
                            ...
                            if (!charBufferAllocate.hasRemaining() || iPosition == 0) {
                                ...
                                int writePosition2 = chunkBuffer2.getWritePosition() - readPosition;
                                ByteBuffer byteBufferM7639slice87lwejk = Memory.m7639slice87lwejk(memory, readPosition, writePosition2);
                                int iLimit = byteBufferM7639slice87lwejk.limit();
                                int iPosition2 = byteBufferM7639slice87lwejk.position();
                                boolean z5 = iLimit - iPosition2 >= iPosition;
                                if (z5) {
                                    byteBufferM7639slice87lwejk.limit(iPosition2 + iPosition);
                                }
                                CoderResult rc = charsetDecoder.decode(byteBufferM7639slice87lwejk, charBufferAllocate, z5);
                                if (rc.isMalformed() || rc.isUnmappable()) {
                                    Intrinsics.checkNotNullExpressionValue(rc, "rc");
                                    throwExceptionWrapped(rc);
                                }
                                i3 = (rc.isUnderflow() && byteBufferM7639slice87lwejk.hasRemaining()) ? i3 + 1 : 1;
                                byteBufferM7639slice87lwejk.limit(iLimit);
                                iPosition -= byteBufferM7639slice87lwejk.position() - iPosition2;
                                if (byteBufferM7639slice87lwejk.limit() != writePosition2) {
                                    throw new IllegalStateException("Buffer's limit change is not allowed".toString());
                                }
                                chunkBuffer2.discardExact(byteBufferM7639slice87lwejk.position());
    ...
    public static final boolean encodeComplete(CharsetEncoder charsetEncoder, Buffer dst) throws CharacterCodingException {
        ...
        int limit = dst.getLimit() - writePosition;
        ByteBuffer byteBufferM7639slice87lwejk = Memory.m7639slice87lwejk(memory, writePosition, limit);
        CoderResult result = charsetEncoder.encode(EmptyCharBuffer, byteBufferM7639slice87lwejk, true);
        if (result.isMalformed() || result.isUnmappable()) {
            Intrinsics.checkNotNullExpressionValue(result, "result");
            throwExceptionWrapped(result);
        }
    ...
    public static final int decodeBuffer(CharsetDecoder charsetDecoder, Buffer input, Appendable out, boolean z, int i) {
        ...
        ByteBuffer byteBufferM7639slice87lwejk = Memory.m7639slice87lwejk(memory, readPosition, writePosition);
        ChunkBuffer chunkBufferBorrow = ChunkBuffer.INSTANCE.getPool().borrow();
        CharBuffer charBufferAsCharBuffer = chunkBufferBorrow.getMemory().asCharBuffer();
        int i2 = 0;
        while (byteBufferM7639slice87lwejk.hasRemaining() && i2 < i) {
            try {
                int iMin = Math.min(charBufferAsCharBuffer.capacity(), i - i2);
                charBufferAsCharBuffer.clear();
                charBufferAsCharBuffer.limit(iMin);
                CoderResult result = charsetDecoder.decode(byteBufferM7639slice87lwejk, charBufferAsCharBuffer, z);
                if (result.isMalformed() || result.isUnmappable()) {
                    Intrinsics.checkNotNullExpressionValue(result, "result");
                    throwExceptionWrapped(result);
                }
