    ...
    public void getNextChunk(LoadingInfo loadingInfo, long j, List<? extends MediaChunk> list, ChunkHolder chunkHolder) {
        ...
        if (playerTrackEmsgHandler == null || !playerTrackEmsgHandler.maybeRefreshManifestBeforeLoadingNextChunk(jMsToUs)) {
            ...
            while (i3 < length) {
                ...
                if (representationHolder.segmentIndex == null) {
                    ...
                    if (segmentNum2 < firstAvailableSegmentNum2) {
                        ...
                        mediaChunkIteratorArr[i] = new RepresentationSegmentIterator(updateSelectedBaseUrl(i), segmentNum2, lastAvailableSegmentNum2, nowPeriodTimeUs);
                    }
                }
                i3 = i + 1;
                jMsToUs2 = j3;
            ...
            if (this.manifest.dynamic) {
                ...
                int iMin = (int) Math.min(this.maxSegmentsPerLoad, (lastAvailableSegmentNum - segmentNum) + 1);
                if (j8 != -9223372036854775807L) {
                    while (iMin > 1 && representationHolderUpdateSelectedBaseUrl.getSegmentStartTimeUs((((long) iMin) + segmentNum) - 1) >= j8) {
                        iMin--;
                    }
                }
                chunkHolder.chunk = newMediaChunk(representationHolderUpdateSelectedBaseUrl, this.dataSource, this.trackType, this.trackSelection.getSelectedFormat(), this.trackSelection.getSelectionReason(), this.trackSelection.getSelectionData(), segmentNum, iMin, list.isEmpty() ? j : -9223372036854775807L, nowPeriodTimeUs, factory);
                return;
            }
            z = true;
            z2 = false;
