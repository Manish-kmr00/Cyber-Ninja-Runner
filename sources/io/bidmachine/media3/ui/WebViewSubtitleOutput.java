...
final class WebViewSubtitleOutput extends FrameLayout implements SubtitleView.Output {
    ...
    /* JADX WARN: Code duplicated, block: B:34:0x013a A[DONT_INVERT] */
    /* JADX WARN: Code duplicated, block: B:35:0x013c A[DONT_INVERT] */
    ...
    private void updateWebView() {
        ...
        for (int i3 = 0; i3 < this.textCues.size(); i3++) {
            ...
            if (cue.line != -3.4028235E38f) {
                if (cue.lineType != 1) {
                    ...
                    if (cue.line >= 0.0f) {
                        invariant2 = Util.formatInvariant("%.2fem", Float.valueOf(cue.line * CSS_LINE_HEIGHT));
                        z = false;
                    } else {
                        z = true;
                        invariant2 = Util.formatInvariant("%.2fem", Float.valueOf(((-cue.line) - 1.0f) * CSS_LINE_HEIGHT));
                    }
                    iAnchorTypeToTranslatePercent = 0;
                }
                if (cue.size != -3.4028235E38f) {
                    invariant3 = Util.formatInvariant("%.2f%%", Float.valueOf(cue.size * 100.0f));
                } else {
                ...
                if (i2 != 1) {
                    ...
                    str3 = str;
                    obj = ViewHierarchyConstants.DIMENSION_TOP_KEY;
                } else if (i2 != 2) {
                    if (z) {
                        str2 = "bottom";
                    }
                ...
                sb.append(Util.formatInvariant("<div style='position:absolute;z-index:%s;%s:%.2f%%;%s:%s;%s:%s;text-align:%s;writing-mode:%s;font-size:%s;background-color:%s;transform:translate(%s%%,%s%%)%s;'>", Integer.valueOf(i3), obj, Float.valueOf(f), str3, invariant2, obj2, str5, strConvertAlignmentToCss, strConvertVerticalTypeToCss, strConvertTextSizeToCss, cssRgba, Integer.valueOf(iAnchorTypeToTranslatePercent2), Integer.valueOf(iAnchorTypeToTranslatePercent), getBlockShearTransformFunction(cue))).append(Util.formatInvariant("<span class='%s'>", DEFAULT_BACKGROUND_CSS_CLASS));
            ...
            sb.append(Util.formatInvariant("<div style='position:absolute;z-index:%s;%s:%.2f%%;%s:%s;%s:%s;text-align:%s;writing-mode:%s;font-size:%s;background-color:%s;transform:translate(%s%%,%s%%)%s;'>", Integer.valueOf(i3), obj, Float.valueOf(f), str3, invariant2, obj2, str7, strConvertAlignmentToCss2, strConvertVerticalTypeToCss2, strConvertTextSizeToCss2, cssRgba2, Integer.valueOf(iAnchorTypeToTranslatePercent2), Integer.valueOf(iAnchorTypeToTranslatePercent), getBlockShearTransformFunction(cue))).append(Util.formatInvariant("<span class='%s'>", DEFAULT_BACKGROUND_CSS_CLASS));
            if (cue.multiRowAlignment != null) {
                sb.append(Util.formatInvariant("<span style='display:inline-block; text-align:%s;'>", convertAlignmentToCss(cue.multiRowAlignment))).append(htmlAndCssConvert.html).append("</span>");
            } else {
                sb.append(htmlAndCssConvert.html);
        ...
        for (String str8 : map.keySet()) {
            sb2.append(str8).append("{").append((String) map.get(str8)).append("}");
        }
        sb2.append("</style></head>");
        sb.insert(0, sb2.toString());
        BidMachineNetworkBridge.webviewLoadData(this.webView, Base64.encodeToString(sb.toString().getBytes(Charsets.UTF_8), 1), POBCommonConstants.CONTENT_TYPE_HTML, "base64");
    }
    ...
    /* JADX INFO: renamed from: io.bidmachine.media3.ui.WebViewSubtitleOutput$2, reason: invalid class name */
