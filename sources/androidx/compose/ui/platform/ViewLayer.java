...
public final class ViewLayer extends View implements OwnedLayer, GraphicLayerInfo {
    ...
    public ViewLayer(AndroidComposeView ownerView, DrawChildContainer container, Function1<? super Canvas, Unit> drawBlock, Function0<Unit> invalidateParentLayer) {
        ...
        this.mTransformOrigin = TransformOrigin.INSTANCE.m1973getCenterSzJe1aQ();
    ...
    public void mo3295updateLayerPropertiesNHXXZp8(float scaleX, float scaleY, float alpha, float translationX, float translationY, float shadowElevation, float rotationX, float rotationY, float rotationZ, float cameraDistance, long transformOrigin, Shape shape, boolean clip, RenderEffect renderEffect, long ambientShadowColor, long spotShadowColor, LayoutDirection layoutDirection, Density density) {
        ...
        setPivotX(TransformOrigin.m1968getPivotFractionXimpl(this.mTransformOrigin) * getWidth());
        setPivotY(TransformOrigin.m1969getPivotFractionYimpl(this.mTransformOrigin) * getHeight());
        ...
        if (Build.VERSION.SDK_INT >= 28) {
            ...
            ViewLayerVerificationHelper28.INSTANCE.setOutlineAmbientShadowColor(viewLayer, ColorKt.m1686toArgb8_81llA(ambientShadowColor));
            ViewLayerVerificationHelper28.INSTANCE.setOutlineSpotShadowColor(viewLayer, ColorKt.m1686toArgb8_81llA(spotShadowColor));
        }
    ...
    public boolean mo3291isInLayerk4lQ0M(long position) {
        float fM1395getXimpl = Offset.m1395getXimpl(position);
        float fM1396getYimpl = Offset.m1396getYimpl(position);
        if (this.clipToBounds) {
            return 0.0f <= fM1395getXimpl && fM1395getXimpl < ((float) getWidth()) && 0.0f <= fM1396getYimpl && fM1396getYimpl < ((float) getHeight());
        }
        if (getClipToOutline()) {
            return this.outlineResolver.m3364isInOutlinek4lQ0M(position);
        }
        return true;
    }
    ...
    public void mo3294resizeozmzZPI(long size) {
        int iM4003getWidthimpl = IntSize.m4003getWidthimpl(size);
        int iM4002getHeightimpl = IntSize.m4002getHeightimpl(size);
        if (iM4003getWidthimpl == getWidth() && iM4002getHeightimpl == getHeight()) {
            return;
        }
        float f = iM4003getWidthimpl;
        setPivotX(TransformOrigin.m1968getPivotFractionXimpl(this.mTransformOrigin) * f);
        float f2 = iM4002getHeightimpl;
        setPivotY(TransformOrigin.m1969getPivotFractionYimpl(this.mTransformOrigin) * f2);
        this.outlineResolver.m3365updateuvyYCjk(SizeKt.Size(f, f2));
        updateOutlineResolver();
        layout(getLeft(), getTop(), getLeft() + iM4003getWidthimpl, getTop() + iM4002getHeightimpl);
        resetClipBounds();
        this.matrixCache.invalidate();
    }
    ...
    public void mo3293movegyyYBs(long position) {
        int iM3961getXimpl = IntOffset.m3961getXimpl(position);
        if (iM3961getXimpl != getLeft()) {
            offsetLeftAndRight(iM3961getXimpl - getLeft());
            this.matrixCache.invalidate();
        }
        int iM3962getYimpl = IntOffset.m3962getYimpl(position);
        if (iM3962getYimpl != getTop()) {
            offsetTopAndBottom(iM3962getYimpl - getTop());
            this.matrixCache.invalidate();
        }
    }
    ...
    public long mo3292mapOffset8S9VItk(long point, boolean inverse) {
        if (inverse) {
            float[] fArrM3341calculateInverseMatrixbWbORWo = this.matrixCache.m3341calculateInverseMatrixbWbORWo(this);
            return fArrM3341calculateInverseMatrixbWbORWo != null ? androidx.compose.ui.graphics.Matrix.m1831mapMKHz9U(fArrM3341calculateInverseMatrixbWbORWo, point) : Offset.INSTANCE.m1409getInfiniteF1C5BW0();
        }
        return androidx.compose.ui.graphics.Matrix.m1831mapMKHz9U(this.matrixCache.m3342calculateMatrixGrdbGEg(this), point);
    }
    ...
    public void mapBounds(MutableRect rect, boolean inverse) {
        ...
        if (inverse) {
            float[] fArrM3341calculateInverseMatrixbWbORWo = this.matrixCache.m3341calculateInverseMatrixbWbORWo(this);
            if (fArrM3341calculateInverseMatrixbWbORWo != null) {
                androidx.compose.ui.graphics.Matrix.m1833mapimpl(fArrM3341calculateInverseMatrixbWbORWo, rect);
                return;
            } else {
        ...
        androidx.compose.ui.graphics.Matrix.m1833mapimpl(this.matrixCache.m3342calculateMatrixGrdbGEg(this), rect);
    }
