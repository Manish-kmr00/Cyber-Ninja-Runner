...
public final class MultiParagraph {
    ...
    private MultiParagraph(MultiParagraphIntrinsics multiParagraphIntrinsics, long j, int i, boolean z) {
        ...
        this.maxLines = i;
        if (Constraints.m3801getMinWidthimpl(j) != 0 || Constraints.m3800getMinHeightimpl(j) != 0) {
        ...
        while (true) {
            ...
            if (Constraints.m3794getHasBoundedHeightimpl(j)) {
                iM3798getMaxHeightimpl = RangesKt.coerceAtLeast(Constraints.m3798getMaxHeightimpl(j) - ParagraphKt.ceilToInt(f), i2);
            } else {
                iM3798getMaxHeightimpl = Constraints.m3798getMaxHeightimpl(j);
            }
            Paragraph paragraphM3416Paragraph_EkL_Y = ParagraphKt.m3416Paragraph_EkL_Y(intrinsics, ConstraintsKt.Constraints$default(0, iM3799getMaxWidthimpl, 0, iM3798getMaxHeightimpl, 5, null), this.maxLines - i3, z);
            float height = f + paragraphM3416Paragraph_EkL_Y.getHeight();
            int lineCount = i3 + paragraphM3416Paragraph_EkL_Y.getLineCount();
            List<ParagraphIntrinsicInfo> list = infoList$ui_text_release;
            arrayList.add(new ParagraphInfo(paragraphM3416Paragraph_EkL_Y, paragraphIntrinsicInfo.getStartIndex(), paragraphIntrinsicInfo.getEndIndex(), i3, lineCount, f, height));
            if (paragraphM3416Paragraph_EkL_Y.getDidExceedMaxLines() || (lineCount == this.maxLines && i4 != CollectionsKt.getLastIndex(this.intrinsics.getInfoList$ui_text_release()))) {
                z2 = true;
                i3 = lineCount;
                f = height;
                break;
        ...
        this.width = Constraints.m3799getMaxWidthimpl(j);
    ...
    public static /* synthetic */ void m3403paintRPmYEkk$default(MultiParagraph multiParagraph, Canvas canvas, long j, Shadow shadow, TextDecoration textDecoration, int i, Object obj) {
        if ((i & 2) != 0) {
            j = Color.INSTANCE.m1667getUnspecified0d7_KjU();
        }
        multiParagraph.m3406paintRPmYEkk(canvas, j, (i & 4) != 0 ? null : shadow, (i & 8) != 0 ? null : textDecoration);
    }

    /* JADX INFO: renamed from: paint-RPmYEkk, reason: not valid java name */
    public final void m3406paintRPmYEkk(Canvas canvas, long color, Shadow shadow, TextDecoration decoration) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            paragraphInfo.getParagraph().mo3410paintRPmYEkk(canvas, color, shadow, decoration);
    ...
    public final Path getPathForRange(int start, int end) {
        ...
        for (int iFindParagraphByIndex = MultiParagraphKt.findParagraphByIndex(this.paragraphInfoList, start); iFindParagraphByIndex < size; iFindParagraphByIndex++) {
            ...
            if (paragraphInfo.getStartIndex() != paragraphInfo.getEndIndex()) {
                Path.m1863addPathUv8p0NA$default(Path, paragraphInfo.toGlobal(paragraphInfo.getParagraph().getPathForRange(paragraphInfo.toLocalIndex(start), paragraphInfo.toLocalIndex(end))), 0L, 2, null);
            }
        }
        return Path;
    }
    ...
    public final int m3404getOffsetForPositionk4lQ0M(long position) {
        int lastIndex;
        if (Offset.m1396getYimpl(position) <= 0.0f) {
            lastIndex = 0;
        } else {
            lastIndex = Offset.m1396getYimpl(position) >= this.height ? CollectionsKt.getLastIndex(this.paragraphInfoList) : MultiParagraphKt.findParagraphByY(this.paragraphInfoList, Offset.m1396getYimpl(position));
        }
        ParagraphInfo paragraphInfo = this.paragraphInfoList.get(lastIndex);
        if (paragraphInfo.getLength() == 0) {
            return Math.max(0, paragraphInfo.getStartIndex() - 1);
        }
        return paragraphInfo.toGlobalIndex(paragraphInfo.getParagraph().mo3408getOffsetForPositionk4lQ0M(paragraphInfo.m3413toLocalMKHz9U(position)));
    }
    ...
    public final long m3405getWordBoundaryjx7JFs(int offset) {
        int iFindParagraphByIndex;
        requireIndexInRangeInclusiveEnd(offset);
        if (offset == getAnnotatedString().length()) {
            iFindParagraphByIndex = CollectionsKt.getLastIndex(this.paragraphInfoList);
        ...
        ParagraphInfo paragraphInfo = this.paragraphInfoList.get(iFindParagraphByIndex);
        return paragraphInfo.m3412toGlobalGEjPoXI(paragraphInfo.getParagraph().mo3409getWordBoundaryjx7JFs(paragraphInfo.toLocalIndex(offset)));
    }
