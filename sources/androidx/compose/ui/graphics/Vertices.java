...
public final class Vertices {
    ...
    private final int[] encodeColorList(List<Color> colors) {
        ...
        for (int i = 0; i < size; i++) {
            iArr[i] = ColorKt.m1686toArgb8_81llA(colors.get(i).m1641unboximpl());
        }
        return iArr;
    }
    ...
    private final float[] encodePointList(List<Offset> points) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (i % 2 == 0) {
                fM1396getYimpl = Offset.m1395getXimpl(packedValue);
            } else {
                fM1396getYimpl = Offset.m1396getYimpl(packedValue);
            }
            fArr[i] = fM1396getYimpl;
        }
        return fArr;
    }
}
