...
public final class AndroidRippleIndicationInstance extends RippleIndicationInstance implements RememberObserver {
    ...
    private AndroidRippleIndicationInstance(boolean z, float f, State<Color> state, State<RippleAlpha> state2, RippleContainer rippleContainer) {
        ...
        this.rippleSize = Size.INSTANCE.m1473getZeroNHjbRc();
        this.rippleRadius = -1;
    ...
    public void drawIndication(ContentDrawScope contentDrawScope) {
        ...
        this.rippleSize = contentDrawScope.mo2125getSizeNHjbRc();
        if (Float.isNaN(this.radius)) {
            iRoundToInt = MathKt.roundToInt(RippleAnimationKt.m1259getRippleEndRadiuscSwnlzA(contentDrawScope, this.bounded, contentDrawScope.mo2125getSizeNHjbRc()));
        } else {
            iRoundToInt = contentDrawScope.mo312roundToPx0680j_4(this.radius);
        }
        this.rippleRadius = iRoundToInt;
        long jM1641unboximpl = this.color.getValue().m1641unboximpl();
        float pressedAlpha = this.rippleAlpha.getValue().getPressedAlpha();
        contentDrawScope.drawContent();
        ContentDrawScope contentDrawScope2 = contentDrawScope;
        m1264drawStateLayerH2RKhps(contentDrawScope2, this.radius, jM1641unboximpl);
        Canvas canvas = contentDrawScope2.getDrawContext().getCanvas();
        getInvalidateTick();
        RippleHostView rippleHostView = getRippleHostView();
        if (rippleHostView != null) {
            rippleHostView.m1263updateRipplePropertiesbiQXAtU(contentDrawScope.mo2125getSizeNHjbRc(), this.rippleRadius, jM1641unboximpl, pressedAlpha);
    ...
    public void addRipple(PressInteraction.Press interaction, CoroutineScope scope) {
        ...
        rippleHostView.m1262addRippleKOepWvA(interaction, this.bounded, this.rippleSize, this.rippleRadius, this.color.getValue().m1641unboximpl(), this.rippleAlpha.getValue().getPressedAlpha(), this.onInvalidateRipple);
