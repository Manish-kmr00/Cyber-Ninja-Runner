...
public final class TextFieldSelectionManagerKt {
    ...
    public static final void TextFieldSelectionHandle(final boolean z, final ResolvedTextDirection direction, final TextFieldSelectionManager manager, Composer composer, final int i) {
        ...
        ComposerKt.sourceInformation(composerStartRestartGroup, "C(TextFieldSelectionHandle)P(1)806@30323L90,811@30479L327:TextFieldSelectionManager.kt#eksfi3");
        ...
        int i2 = i << 3;
        AndroidSelectionHandles_androidKt.m781SelectionHandle8fL75g(manager.m869getHandlePositiontuRUvjQ$foundation_release(z), z, direction, TextRange.m3488getReversedimpl(manager.getValue$foundation_release().getSelection()), SuspendingPointerInputFilterKt.pointerInput(Modifier.INSTANCE, textDragObserver, new AnonymousClass1(textDragObserver, null)), null, composerStartRestartGroup, (i2 & 112) | ProfileVerifier.CompilationStatus.RESULT_CODE_ERROR_CANT_WRITE_PROFILE_VERIFICATION_RESULT_CACHE_FILE | (i2 & 896));
    ...
    public static final boolean isSelectionHandleInVisibleBound(TextFieldSelectionManager textFieldSelectionManager, boolean z) {
        ...
        return SelectionManagerKt.m845containsInclusiveUv8p0NA(rectVisibleBounds, textFieldSelectionManager.m869getHandlePositiontuRUvjQ$foundation_release(z));
    }

    /* JADX INFO: renamed from: calculateSelectionMagnifierCenterAndroid-O0kMr_c, reason: not valid java name */
    public static final long m870calculateSelectionMagnifierCenterAndroidO0kMr_c(TextFieldSelectionManager manager, long j) {
        int iM3489getStartimpl;
        ...
        if (i == 1 || i == 2) {
            iM3489getStartimpl = TextRange.m3489getStartimpl(manager.getValue$foundation_release().getSelection());
        } else {
            if (i != 3) {
                throw new NoWhenBranchMatchedException();
            }
            iM3489getStartimpl = TextRange.m3484getEndimpl(manager.getValue$foundation_release().getSelection());
        }
        int iCoerceIn = RangesKt.coerceIn(manager.getOffsetMapping().originalToTransformed(iM3489getStartimpl), (ClosedRange<Integer>) StringsKt.getIndices(manager.getValue$foundation_release().getText()));
        TextFieldState state$foundation_release = manager.getState();
        if (state$foundation_release == null || (layoutResult = state$foundation_release.getLayoutResult()) == null || (value = layoutResult.getValue()) == null) {
            return Offset.INSTANCE.m1410getUnspecifiedF1C5BW0();
        }
        long jM1425getCenterF1C5BW0 = value.getBoundingBox(iCoerceIn).m1425getCenterF1C5BW0();
        ...
        Offset offsetM867getCurrentDragPosition_m7T9E = manager.m867getCurrentDragPosition_m7T9E();
        if (offsetM867getCurrentDragPosition_m7T9E == null) {
            return Offset.INSTANCE.m1410getUnspecifiedF1C5BW0();
        }
        float fM1395getXimpl = Offset.m1395getXimpl(innerTextFieldCoordinates.mo3127localPositionOfR5De75A(layoutCoordinates, offsetM867getCurrentDragPosition_m7T9E.getPackedValue()));
        ...
        boolean z = TextRange.m3489getStartimpl(manager.getValue$foundation_release().getSelection()) > TextRange.m3484getEndimpl(manager.getValue$foundation_release().getSelection());
        ...
        float fCoerceIn = RangesKt.coerceIn(fM1395getXimpl, Math.min(horizontalPosition, horizontalPosition2), Math.max(horizontalPosition, horizontalPosition2));
        if (Math.abs(fM1395getXimpl - fCoerceIn) > IntSize.m4003getWidthimpl(j) / 2) {
            return Offset.INSTANCE.m1410getUnspecifiedF1C5BW0();
        }
        return layoutCoordinates.mo3127localPositionOfR5De75A(innerTextFieldCoordinates, OffsetKt.Offset(fCoerceIn, Offset.m1396getYimpl(jM1425getCenterF1C5BW0)));
    }
