...
public final class TextLayoutHelperKt {
    ...
    public static final boolean m768canReuse7_7YC6M(TextLayoutResult canReuse, AnnotatedString text, TextStyle style, List<AnnotatedString.Range<Placeholder>> placeholders, int i, boolean z, int i2, Density density, LayoutDirection layoutDirection, FontFamily.Resolver fontFamilyResolver, long j) {
        ...
        if (canReuse.getMultiParagraph().getIntrinsics().getHasStaleResolvedFonts() || !Intrinsics.areEqual(layoutInput.getText(), text) || !layoutInput.getStyle().hasSameLayoutAffectingAttributes(style) || !Intrinsics.areEqual(layoutInput.getPlaceholders(), placeholders) || layoutInput.getMaxLines() != i || layoutInput.getSoftWrap() != z || !TextOverflow.m3777equalsimpl0(layoutInput.getOverflow(), i2) || !Intrinsics.areEqual(layoutInput.getDensity(), density) || layoutInput.getLayoutDirection() != layoutDirection || !Intrinsics.areEqual(layoutInput.getFontFamilyResolver(), fontFamilyResolver) || Constraints.m3801getMinWidthimpl(j) != Constraints.m3801getMinWidthimpl(layoutInput.getConstraints())) {
            return false;
        }
        if (z || TextOverflow.m3777equalsimpl0(i2, TextOverflow.INSTANCE.m3785getEllipsisgIe3tQ8())) {
            return Constraints.m3799getMaxWidthimpl(j) == Constraints.m3799getMaxWidthimpl(layoutInput.getConstraints()) && Constraints.m3798getMaxHeightimpl(j) == Constraints.m3798getMaxHeightimpl(layoutInput.getConstraints());
        }
        return true;
    }
}
