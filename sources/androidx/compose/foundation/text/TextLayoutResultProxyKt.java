...
public final class TextLayoutResultProxyKt {
    ...
    public static final long m775coerceIn3MmeM6k(long j, Rect rect) {
        float right;
        float bottom;
        if (Offset.m1395getXimpl(j) < rect.getLeft()) {
            right = rect.getLeft();
        } else {
            right = Offset.m1395getXimpl(j) > rect.getRight() ? rect.getRight() : Offset.m1395getXimpl(j);
        }
        if (Offset.m1396getYimpl(j) < rect.getTop()) {
            bottom = rect.getTop();
        } else {
            bottom = Offset.m1396getYimpl(j) > rect.getBottom() ? rect.getBottom() : Offset.m1396getYimpl(j);
        }
