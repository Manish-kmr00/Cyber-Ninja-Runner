...
public final class LazyListHeadersKt {
    public static final LazyListPositionedItem findOrComposeLazyListHeader(List<LazyListPositionedItem> composedVisibleItems, LazyMeasuredItemProvider itemProvider, List<Integer> headerIndexes, int i, int i2, int i3) {
        ...
        LazyMeasuredItem lazyMeasuredItemM553getAndMeasureZjPyQlc = itemProvider.m553getAndMeasureZjPyQlc(DataIndex.m512constructorimpl(iIntValue));
        int iMax = offset != Integer.MIN_VALUE ? Math.max(-i, offset) : -i;
        if (offset2 != Integer.MIN_VALUE) {
            iMax = Math.min(iMax, offset2 - lazyMeasuredItemM553getAndMeasureZjPyQlc.getSize());
        }
        LazyListPositionedItem lazyListPositionedItemPosition = lazyMeasuredItemM553getAndMeasureZjPyQlc.position(iMax, i2, i3);
        if (i5 != -1) {
