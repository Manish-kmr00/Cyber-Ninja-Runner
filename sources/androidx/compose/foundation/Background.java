...
final class Background extends InspectorValueInfo implements DrawModifier {
    ...
    private final void drawRect(ContentDrawScope contentDrawScope) {
        ...
        if (color != null) {
            DrawScope.m2120drawRectnJ9OG0$default(contentDrawScope, color.m1641unboximpl(), 0L, 0L, 0.0f, null, null, 0, 126, null);
        }
        Brush brush = this.brush;
        if (brush != null) {
            DrawScope.m2119drawRectAsUm42w$default(contentDrawScope, brush, 0L, 0L, this.alpha, null, null, 0, Sdk.SDKError.Reason.PROTOBUF_SERIALIZATION_ERROR_VALUE, null);
        }
    }

    private final void drawOutline(ContentDrawScope contentDrawScope) {
        Outline outlineMo211createOutlinePq9zytI;
        if (Size.m1459equalsimpl(contentDrawScope.mo2125getSizeNHjbRc(), this.lastSize) && contentDrawScope.getLayoutDirection() == this.lastLayoutDirection) {
            outlineMo211createOutlinePq9zytI = this.lastOutline;
            Intrinsics.checkNotNull(outlineMo211createOutlinePq9zytI);
        } else {
            outlineMo211createOutlinePq9zytI = this.shape.mo211createOutlinePq9zytI(contentDrawScope.mo2125getSizeNHjbRc(), contentDrawScope.getLayoutDirection(), contentDrawScope);
        }
        Color color = this.color;
        if (color != null) {
            color.m1641unboximpl();
            OutlineKt.m1852drawOutlinewDX37Ww(contentDrawScope, outlineMo211createOutlinePq9zytI, this.color.m1641unboximpl(), (60 & 4) != 0 ? 1.0f : 0.0f, (60 & 8) != 0 ? Fill.INSTANCE : null, (60 & 16) != 0 ? null : null, (60 & 32) != 0 ? DrawScope.INSTANCE.m2126getDefaultBlendMode0nO6VwU() : 0);
        }
        Brush brush = this.brush;
        if (brush != null) {
            OutlineKt.m1851drawOutlinehn5TExg$default(contentDrawScope, outlineMo211createOutlinePq9zytI, brush, this.alpha, null, null, 0, 56, null);
        }
        this.lastOutline = outlineMo211createOutlinePq9zytI;
        this.lastSize = Size.m1452boximpl(contentDrawScope.mo2125getSizeNHjbRc());
    }

    public int hashCode() {
        Color color = this.color;
        int iM1638hashCodeimpl = (color != null ? Color.m1638hashCodeimpl(color.m1641unboximpl()) : 0) * 31;
        Brush brush = this.brush;
        return ((((iM1638hashCodeimpl + (brush != null ? brush.hashCode() : 0)) * 31) + Float.hashCode(this.alpha)) * 31) + this.shape.hashCode();
    }
