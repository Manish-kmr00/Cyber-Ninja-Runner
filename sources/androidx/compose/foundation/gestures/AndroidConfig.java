...
final class AndroidConfig implements ScrollConfig {
    ...
    public long mo248calculateMouseWheelScroll8xgXZGE(Density calculateMouseWheelScroll, PointerEvent event, long j) {
        Intrinsics.checkNotNullParameter(calculateMouseWheelScroll, "$this$calculateMouseWheelScroll");
        Intrinsics.checkNotNullParameter(event, "event");
        List<PointerInputChange> changes = event.getChanges();
        Offset offsetM1384boximpl = Offset.m1384boximpl(Offset.INSTANCE.m1411getZeroF1C5BW0());
        int size = changes.size();
        for (int i = 0; i < size; i++) {
            offsetM1384boximpl = Offset.m1384boximpl(Offset.m1400plusMKHz9U(offsetM1384boximpl.getPackedValue(), changes.get(i).getScrollDelta()));
        }
        return Offset.m1402timestuRUvjQ(offsetM1384boximpl.getPackedValue(), -calculateMouseWheelScroll.mo318toPx0680j_4(Dp.m3843constructorimpl(64)));
    }
}
