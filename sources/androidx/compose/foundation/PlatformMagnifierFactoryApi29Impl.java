...
public final class PlatformMagnifierFactoryApi29Impl implements PlatformMagnifierFactory {
    ...
    public PlatformMagnifierImpl create(MagnifierStyle style, View view, Density density, float initialZoom) {
        ...
        long jMo319toSizeXkaWNTQ = density.mo319toSizeXkaWNTQ(style.getSize());
        float fMo318toPx0680j_4 = density.mo318toPx0680j_4(style.getCornerRadius());
        float fMo318toPx0680j_5 = density.mo318toPx0680j_4(style.getElevation());
        Magnifier.Builder builder = new Magnifier.Builder(view);
        if (jMo319toSizeXkaWNTQ != Size.INSTANCE.m1472getUnspecifiedNHjbRc()) {
            builder.setSize(MathKt.roundToInt(Size.m1464getWidthimpl(jMo319toSizeXkaWNTQ)), MathKt.roundToInt(Size.m1461getHeightimpl(jMo319toSizeXkaWNTQ)));
        }
        if (!Float.isNaN(fMo318toPx0680j_4)) {
            builder.setCornerRadius(fMo318toPx0680j_4);
        }
        if (!Float.isNaN(fMo318toPx0680j_5)) {
            builder.setElevation(fMo318toPx0680j_5);
        }
    ...
    public static final class PlatformMagnifierImpl extends PlatformMagnifierFactoryApi28Impl.PlatformMagnifierImpl {
        ...
        public void mo247updateWko1d7g(long sourceCenter, long magnifierCenter, float zoom) {
            if (!Float.isNaN(zoom)) {
                getMagnifier().setZoom(zoom);
            }
            if (OffsetKt.m1414isSpecifiedk4lQ0M(magnifierCenter)) {
                getMagnifier().show(Offset.m1395getXimpl(sourceCenter), Offset.m1396getYimpl(sourceCenter), Offset.m1395getXimpl(magnifierCenter), Offset.m1396getYimpl(magnifierCenter));
            } else {
                getMagnifier().show(Offset.m1395getXimpl(sourceCenter), Offset.m1396getYimpl(sourceCenter));
            }
        }
    }
}
